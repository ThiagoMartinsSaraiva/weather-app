  ...
  actions: {
    storeCity(city: any) {
      const foundCity = this.storedCities.find((cityItem: any) => {
        return cityItem.location.lat == city.location.lat && cityItem.location.lon == city.location.lon
      })
      ...
      localStorage.setItem('tms-weather-app/cities', JSON.stringify(this.storedCities))
    },
    removeCity(city: any) {
